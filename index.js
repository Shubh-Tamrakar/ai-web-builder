...
async function executeCommand({ command }) {
  try {
    ...
    if (
      ...
      return "Error: File redirection missing. Use: cat << 'EOF' > filename";
    }
    ...
    return stdout
      ? `Success:\n${stdout}`
      : "Success: Command executed, file written";
...
async function runAgent(userProblem) {
   ...
   const response = await ai.models.generateContent({
    ...
    config: {
        ...
        If OS is win32:
- Use: mkdir folder
- Use: type nul > index.html

If OS is linux or darwin:
- Use: touch index.html
        ...
        When writing code into a file, ALWAYS use this exact format:
...
NEVER omit "> filename".
NEVER print code without redirecting to a file.
